...
- Branch `codex/perf-001-stage-timing` is stacked on ALG-001 commit `e1872c6`.
- Runtime/API timing uses `hcns-stage-timing/1.0.0`; private timing sidecars and
  the benchmark report contain numbers only, never source content or identity.
- DATA-29 authorized local run completed 4 cold + 120 warm measurements. Warm
  total p50/p95: DOCX `167/285 ms`, PDF text `134/158 ms`, image `19.5/28.5 s`,
  PDF scan `33.1/67.5 s`; no benchmark failure remained in the final report.
- Running an already-loaded EasyOCR API beside scan benchmarking caused one
  1.20 GB allocation failure on the 8 GB host. Separate processes completed the
  scan and image cohorts; this is a capacity HOLD, not an extraction regression.
- Live Camunda timing smoke passed at `1.215 s`; aggregate Camunda percentile is
  explicitly `NOT_MEASURED` because no 31-case benchmark cohort was created.
- DATA-29 quality remains `107/112` strict and `112/112` accepted. Do not open
  CCCD or Contract images. Implement ALG-002 next, then use the same runner for
  PDF-001 optimization and quality/performance gates.
- Private report: `C:\tmp\hcns-perf-001-data29-20260813-v2\aggregate-report.json`.
