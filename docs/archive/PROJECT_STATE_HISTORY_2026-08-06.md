# Project State History (archived 2026-08-06)
...
## DATA-14 checkpoint (2026-08-05, IN PROGRESS)

- Opened local OCR scope for CV, employment contract, contract appendix and HR
  decision image/PDF-scan inputs. OCR outputs remain `MANUAL_REVIEW`.
- Replaced external CV/contract/certificate line-flattening fallbacks with the
  shared Phase 17 bounded-label, section and evidence-aware helpers. Added
  `extractor`, `method`, `sourceSpan` and `reviewReason` to mapped fields.
- DATA-13 policy now reports `all-active-families`; existing evaluate-once
  artifacts were not overwritten or reopened.
- Added regression coverage for multiline CV/contract fields and visual-family
  scoring. Targeted tests: 21 passed; full Python suite: 316 passed; mypy:
  85 source files passed; touched-file Ruff passed.
- Local API restarted successfully on loopback and `/health` returned `ok`.
- A fresh Paddle CPU prediction-only dry-run was started outside Git but
  stopped after model startup/partial processing exceeded the bounded check;
  no Ground Truth or evaluate-once marker was touched.

Next:
- Run a bounded development split with the Vietnamese local recognizer and
  compare parser recovery before any new evaluate-once artifact.
- Benchmark an installed local document-understanding fallback only after a
  model/runtime is available; do not promote based on the current artifact.

## DATA-15 checkpoint (2026-08-05, DONE/DEV HOLD)

- Imported the user-provided `D:\bo_10_file_contract_cv_ielts_v2` bundle into a
  new private staging root outside Git: 3 Contract, 5 CV and 4 IELTS files;
  13 pages total. No Ground Truth was supplied, so labels were independently
  reviewed from the source documents and sealed locally after all 112 fields
  were confirmed.
- New split/inventory and prediction artifacts are private under `C:\tmp`:
  `bo10-dev-split-20260805.json`,
  `bo10-dev-predictions-data13-private-20260805.json`,
  `bo10-dev-ground-truth-draft-20260805.json`,
  `bo10-dev-ground-truth-draft-20260805-SEALED.json` and
  `bo10-dev-aggregate-comparison-20260805.json`.
- Local DATA-13 prediction completed for 12/12 documents with
  `all-active-families`: 112 schema fields, 48 present (42.86%), 64
  `not_found`, 6 OCR `needs_review`; 5 OCR documents remain `MANUAL_REVIEW`
  and false `AUTO_CONTINUE=0`. Declared-category routing agreed for 11/12;
  this is a routing diagnostic, not Ground Truth classification accuracy.
- The separate development aggregate comparison scored 47/112 exact fields
  (41.96%): Contract 30/42, CV 17/50, IELTS 0/20; classification agreement
  was 11/12, schema errors 0. Diagnostics were
  `PARSER_MISSED=13`, `OCR_RECOGNIZED_PARSER_MISSED=9` and
  `OCR_NOT_RECOGNIZED=23`. The report remains `HOLD` with
  `promotionAllowed=false`.
- Ground Truth seal records `SEALED`, 112 confirmed fields and
  `predictionsOpened=false`. The development report records
  `evaluateOnceMarkerTouched=false`; the original DATA-13 evaluate-once
  artifacts were not read or modified.

Next:
- Use the field-level diagnostics to fix IELTS OCR/layout and contract/CV
  label mapping before creating a held-out split. Keep all images/PDF scans on
  `MANUAL_REVIEW` and do not promote this development result.

## OCR-HO-V2-014 replay checkpoint (2026-08-05, HOLD)

- Replayed the 15-document development split with four existing recognizers
  after fixing the coordinate root cause: phase11 pages and detector boxes
  were already aligned, so the extra perspective transform was skipped when
  boxes are in bounds. Line crops now trim only overlapping previous-line
  pixels; expiry in another column is preserved.
- Final private replay completed 212 jobs. Aggregate candidate metrics versus
  11.9.1: strict 63.33% vs 60.83%, ASCII 69.17% vs 62.50%, CER 30.62% vs
  42.09%, DER 16.60% vs 11.46%, presence 95.83%, region selection 88.33%.
  Target fields: fullName ASCII 93.33%/ROI 100%, placeOfOrigin ASCII 40%/ROI
  86.67%, placeOfResidence ASCII 6.67%/ROI 66.67%. Exact improvements=4,
  regressions=1; both gates remain HOLD because DER and exact-regression
  checks fail and address readiness is below 85%.
- Warning telemetry for this replay: EasyOCR scalar overflow 90, torch
  `weights_only=False` 2, torch nested-tensor 1, VietOCR invalid-divide 0.
  Worker status is `COMPLETE`; no automatic acceptance was enabled.
- Prediction-blind GT remains local-only
  (`DRAFT_NEEDS_LINE_CONFIRMATION`, `linesChecked=false`, prediction unopened).
  Overlays and the draft were regenerated after the final replay; do not seal
  until a human confirms all 15 line mappings in the localhost review UI.
- Lexicon reranking, a fifth OCR engine and fine-tuning remain intentionally
  deferred: residence ROI is below 80% and confirmed line-crop volume is below
  500. Next action is to review the five remaining residence label-contamination
  cases and the one region mismatch, then rerun only after explicit line-ID
  confirmation.

## DATA-15-PREDICTION-INSPECTOR checkpoint (2026-08-05, DONE/DEV HOLD)

- The user did not supply Ground Truth. The 41.96% comparison in DATA-15 is a
  provisional private source-reviewed annotation, not an authoritative accuracy
  claim or promotion gate.
- Localhost now serves the 12-document prediction artifact only:
  `PREDICTION_READY`, `reportAvailable=false`, `predictionBlind=true`,
  `Field exact=—`, and `promotionAllowed=false`. The consumed DATA-13
  evaluate-once marker was not reopened or modified.
- `http://localhost:3000/workspace` → `DATA-13 · OCR scope` shows each predicted
  field with status, method, source span/bbox and review reason where available.
  Native documents use parser evidence; scan/image documents stay OCR/manual
  review only.
- Validation: API `/health` returned `ok`, the prediction summary returned all
  12 documents, web HTTP returned 200, and web lint reported 0 errors (23
  existing warnings).

Next: review predictions against the source files and provide/correct
authoritative Ground Truth; only then run a separate development aggregate.

## DATA-15-GT-REVIEW checkpoint (2026-08-05, READY/DRAFT)

- Created a fresh prediction-blind official Ground Truth draft outside Git at
  `C:\tmp\bo10-official-ground-truth-draft-20260805.json`. It contains the
  12-document field contract (112 fields), all fields `PENDING`/`null`, dataset
  status `DRAFT`, and no prediction values.
- The API now points Ground Truth review endpoints to that draft while keeping
  the DATA-13 prediction artifact separate. Review summary is `DRAFT`,
  `0/112` confirmed and `canLock=false`; no benchmark or evaluate-once artifact
  was run or modified.
- The existing local review panel is available at
  `http://localhost:3000/workspace` → `DATA-08 · 4 contract case review`.
  It shows the source preview, editable field inputs, `Không có / không đọc
  được` checkboxes, per-case save/confirm, and a final SEALED action. The old
  browser test draft was discarded; contract-001 starts at `0/14`.

Next: enter and confirm all 112 source-grounded fields in the review panel;
SEALED is intentionally disabled until every active field is confirmed.

## DATA-15-GT-SEALED checkpoint (2026-08-06, READY FOR DEV AGGREGATE)

- The official prediction-blind Ground Truth was confirmed and sealed through
  the localhost review panel: 12/12 documents, 112/112 fields, dataset status
  `SEALED`, review status `CONFIRMED`.
- Private seal metadata is at
  `C:\tmp\bo10-official-ground-truth-draft-20260805-SEALED.json` and records
  `predictionsOpened=false`. No benchmark, aggregate comparison or old
  evaluate-once artifact was run or changed.

Next: freeze the GroundTruth/prediction paths and run one separate
development-only aggregate comparison. Do not reopen or alter the sealed file.

## DATA-15-DEV-AGGREGATE checkpoint (2026-08-06, HOLD)

- The separate development comparison used the sealed GroundTruth and the
  existing DATA-13 prediction artifact; the old evaluate-once artifact was not
  read or changed.
- Strict normalized exact match is `30/112` (`26.79%`). The explicit text policy
  accepts case-insensitive text and at least 80% source-token coverage only for
  long CV/Contract text fields: `43/112` (`38.39%`). Classification is `11/12`
  (`91.67%`), schema errors are `0`, and OCR remains manual-review-only.
- By family (accepted policy): Contract `16/42` (`38.10%`), CV `27/50`
  (`54.00%`), IELTS/certificate `0/20` (`0%`).
- Private report and marker:
  ...
  DATA-13 localhost now shows `DEVELOPMENT_EVALUATED`, field comparisons and
  source evidence. Decision remains `HOLD`; this is not a promotion result.

## DATA-16-PARSER-V2 checkpoint (2026-08-06, DONE/DEV HOLD)

- Contract/CV layout parsing now uses section-aware extraction, multi-column CV
  header/contact handling, Contract narrative/multiline labels, Vietnamese
  date normalization and schedule-derived weekly hours.
- Fresh private artifacts:
  ...
  The sealed GroundTruth, previous report and old evaluate-once artifact were
  not read or modified.
- Results: strict exact `69/112` (`61.61%`); accepted text `82/112`
  (`73.21%`); classification `11/12` (`91.67%`); schema errors `0`.
  Contract accepted `40/42` (`95.24%`), CV accepted `42/50` (`84%`), IELTS/
  certificate `0/20`; decision `HOLD` and scan/image documents remain manual
  review.
- DATA-13 localhost is connected to parser-v2 and shows per-field
  GroundTruth, prediction, evidence and diagnosis.

Next: improve certificate OCR and scanned-CV OCR, then run another fresh
development comparison only. Do not reopen sealed GroundTruth/evaluate-once.
