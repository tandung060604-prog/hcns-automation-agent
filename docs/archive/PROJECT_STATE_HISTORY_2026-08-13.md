...
- Template-first now emits additive `hcns-stage-timing/1.0.0` metadata for
  intake, OCR, template parsing/validation, private persistence and total time.
  Camunda start responses expose the same safe schema for the handoff stage.
...
- Warm total p50/p95 on this 8 GB CPU machine: DOCX `167/285 ms`, PDF text
  ...
  two visual classes; native OCR timing is `0` by design.
...
- One live Camunda 7.13 start smoke returned safe timing metadata in `1.215 s`.
  Camunda p50/p95 remains `NOT_MEASURED` because this run did not create 31
  additional Human Review cases only for benchmarking.
- Quality evidence remains the pinned DATA-29 `107/112` strict and `112/112`
  accepted; PERF-001 did not rerun or modify Ground Truth. Do not expand CCCD or
  Contract images yet. Next READY is ALG-002, followed by PDF-001 memory/latency.
