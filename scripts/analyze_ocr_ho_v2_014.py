...
def make_documents(root: Path, sealed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for record in sealed["documents"]:
        ...
        artifact = json.loads((session / "phase11_10_v2" / "field_consensus.json").read_text(encoding="utf-8"))
        ...
        documents.append({
            ...
            "phase11_10": artifact.get("identityCard", {}).get("fields", {}),
            "candidateArtifact": artifact,
            "gtFields": record["fields"],
            "durations": {"baseline": result.get("phase11_5", {}).get("durationMs", 0.0), "phase11_10": result.get("phase11_10_v2", {}).get("durationMs", 0.0)},
        })
    return documents
...
def class_report(documents: list[dict[str, Any]], variant: str) -> dict[str, Any]:
    ...
    roi_hits = {name: 0 for name in TARGET_FIELDS}
    evaluated = {name: 0 for name in TARGET_FIELDS}
    for document in documents:
        artifact = document["candidateArtifact"]
        for name in TARGET_FIELDS:
            expected = str(document["groundTruth"].get(name) or "")
            field = (document[variant].get(name) or {})
            if variant == "phase11_10":
                ...
                candidates = [item for item in (artifact.get("candidates", {}).get(name) or []) if isinstance(item, dict)]
            else:
                roi_hit = bool((field.get("evidence") or {}).get("bbox"))
                candidates = [item for item in (field.get("evidence", {}).get("candidates") or []) if isinstance(item, dict)]
            evaluated[name] += 1
    ...
    return {
        ...
        "roiByField": {name: {"correct": roi_hits[name], "evaluated": evaluated[name], "accuracy": round(roi_hits[name] / max(1, evaluated[name]), 6)} for name in TARGET_FIELDS},
    }
...
def main() -> int:
    ...
    schema_errors = sum(1 for document in documents for name in FIELD_ORDER if not isinstance(document["phase11_10"].get(name), dict))
    ...
    keys = ("value", "asciiValue", "confidence", "errorSignals", "evidence", "selectionMode", "validation")
    protected_regressions = sum(
        {key: document["baseline"].get(name, {}).get(key) for key in keys} != {key: document["phase11_10"].get(name, {}).get(key) for key in keys}
        for document in documents for name in PROTECTED_FIELDS
    )
    ...
    report = {
        ...
        "errorAnalyzer": {"classes": CLASS_NAMES, "baseline11_5": class_report(documents, "baseline"), "candidate11_10": class_report(documents, "phase11_10")},
        "gates": gates(before, after, improvements=improvements, regressions=regressions, schema_errors=schema_errors, all_manual_review=all_manual, protected_regressions=protected_regressions),
        ...
        "replay": {"recognizers": ["paddle_ppocrv5", "easyocr_vi", "vietocr_vgg_seq2seq", "vietocr_vgg_transformer"], "lineMapping": "sealed_ground_truth_line_ids", "resultArtifact": "PHASE11_10_V2_RESULTS.json"},
    }
