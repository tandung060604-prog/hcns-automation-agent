#!/usr/bin/env python3
"""Evaluate OCR-HO-V2-008 on the private 15-document development replay."""
...
try:
    ...
    def validate(instance: dict[str, Any], _schema: dict[str, Any]) -> None:
        ...
        if set(instance) != required:
            raise ValidationError("schema envelope")
        ...
        if instance.get("extractionPolicy") != "phase11.6-cccd-address-lines-name-selection":
...
from phase11_8_cccd_v2 import (  # noqa: E402
    CANDIDATE_VERSION,
    POLICY_ID,
    PROTECTED_FIELDS,
    TARGET_FIELDS,
...
def load_documents(
    ...
    for record in manifest.get("records", []):
        ...
        paths = {
            ...
            "candidate": session / "phase11_8_v2" / "field_consensus.json",
        ...
        documents.append(
            {
                ...
                "ocr_ho_v2_008": candidate_fields,
                "candidateCard": candidate_card,
                "durations": {
                    "phase11_5": result.get("phase11_5", {}).get("durationMs") or 0.0,
                    "ocr_ho_v2_008": result.get("phase11_8_v2", {}).get("durationMs") or 0.0,
                },
            }
        )
...
def exact_delta(
    ...
    for document in documents:
        for field_name in FIELD_ORDER:
            ...
            after = value(document["ocr_ho_v2_008"].get(field_name)) == expected
...
def build_gate(
    ...
    manual_review = all(
        ...
        for document in documents
        for field in document["ocr_ho_v2_008"].values()
    )
    checks = {
...
def markdown(report: dict[str, Any]) -> str:
    ...
    candidate = report["metrics"]["ocr_ho_v2_008"]
    ...
    return "\n".join(
        [
            "# OCR-HO-V2-008 Development Comparison",
            "",
            ...
            "",
            "| Metric | Baseline Phase 11.5 | OCR-HO-V2-008 |",
            "|---|---:|---:|",
...
def main() -> int:
    ...
    json_path = report_dir / "CCCD_OCR_HO_V2_008_DEVELOPMENT_COMPARISON.json"
    md_path = report_dir / "CCCD_OCR_HO_V2_008_DEVELOPMENT_COMPARISON.md"
    ...
    candidate = evaluate_variant(documents, "ocr_ho_v2_008")
    report = {
        "schemaVersion": "ocr-ho-v2-008-evaluation/1.0.0",
