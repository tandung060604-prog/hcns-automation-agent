#!/usr/bin/env python3
"""Evaluate OCR-HO-V2-007 address ROI/Unicode recovery on private dev data.

This is a development regression report only.  Ground Truth is read by the
evaluator for scoring and is never supplied to the candidate selector.  The
report contains aggregate metrics and no OCR values or PII.
"""
...
except ModuleNotFoundError:  # pragma: no cover - minimal replay environments
    ValidationError = ValueError  # type: ignore[assignment,misc]

    def validate(instance: dict[str, Any], _schema: dict[str, Any]) -> None:
        """Validate the locked card envelope when jsonschema is unavailable."""
        ...
        if set(instance) != required:
            raise ValidationError("schema envelope keys")
        if instance["schemaVersion"] != "11.6.0":
            raise ValidationError("schema version")
        if instance["documentType"] != "VIETNAM_CITIZEN_ID_FRONT":
            raise ValidationError("document type")
        if instance["extractionPolicy"] != "phase11.6-cccd-address-lines-name-selection":
            raise ValidationError("extraction policy")
        if instance["policyMode"] != "SHADOW_REVIEW_ONLY":
            raise ValidationError("policy mode")
        if set(instance["fields"]) != set(FIELD_ORDER):
            raise ValidationError("field keys")
        modes = {
            "phase11_6_exact_consensus",
            "phase11_6_base_text_consensus",
            "phase11_6_single_candidate",
            "phase11_6_baseline_preserved",
        }
        statuses = {"accepted", "needs_review", "not_found"}
        ascii_statuses = {"verified_base_text", "needs_review", "not_found"}
        for field in instance["fields"].values():
            if not isinstance(field, dict):
                raise ValidationError("field type")
            if not {
                "value", "asciiValue", "status", "asciiStatus", "confidence",
                "errorSignals", "selectionMode", "evidence",
            }.issubset(field):
                raise ValidationError("field required keys")
            if field["status"] not in statuses or field["asciiStatus"] not in ascii_statuses:
                raise ValidationError("field status")
            if not isinstance(field["confidence"], (int, float)) or not (
                0 <= field["confidence"] <= 1
            ):
                raise ValidationError("field confidence")
            if field["selectionMode"] not in modes or not isinstance(field["evidence"], dict):
                raise ValidationError("field selection")
            if not {"pageIndex", "bbox", "candidates"}.issubset(field["evidence"]):
                raise ValidationError("field evidence")
...
def session_root(data_root: Path) -> Path:
    for candidate in (data_root / "user_uploads-sessions", data_root / "user_uploads" / "sessions"):
        if candidate.is_dir():
            return candidate
...
def load_documents(
    ...
    for record in manifest.get("records", []):
        ...
        paths = {
            ...
            "candidate": session / "phase11_7_v2" / "field_consensus.json",
        ...
        documents.append(
            {
                ...
                "ocr_ho_v2_007": candidate_fields,
                "candidateCard": candidate_card,
                "durations": {
                    "phase11_5": result.get("phase11_5", {}).get("durationMs") or 0.0,
                    "ocr_ho_v2_007": result.get("phase11_7_v2", {}).get("durationMs") or 0.0,
                },
            }
        )
    return documents, rotations, skipped


def exact_delta(documents: list[dict[str, Any]]) -> tuple[int, int, dict[str, int], dict[str, int]]:
    ...
    for document in documents:
        for field_name in FIELD_ORDER:
            ...
            after = value(document["ocr_ho_v2_007"].get(field_name)) == expected
...
def build_gate(
    ...
    manual_review = all(
        ...
        for document in documents
        for field in document["ocr_ho_v2_007"].values()
    )
    checks = {
...
def markdown(report: dict[str, Any]) -> str:
    ...
    candidate = report["metrics"]["ocr_ho_v2_007"]
    ...
    return "\n".join(
        [
            "# OCR-HO-V2-007 Development Comparison",
            "",
            ...
            "- Production promotion: **not allowed**; shadow development evaluation only.",
            "- Ground Truth was scoring-only; candidate selection used OCR ROI evidence.",
            "",
            "| Metric | Baseline Phase 11.5 | OCR-HO-V2-007 |",
            "|---|---:|---:|",
            ...
            (
                ...
                f"{gate['exactImprovementCount']}/{gate['exactRegressionCount']}."
            ),
            (
                f"- Schema errors: {gate['schemaErrorCount']}; "
                f"manual-review fields: {gate['manualReviewFieldCount']}."
            ),
...
def main() -> int:
    ...
    json_path = report_dir / "CCCD_OCR_HO_V2_007_DEVELOPMENT_COMPARISON.json"
    md_path = report_dir / "CCCD_OCR_HO_V2_007_DEVELOPMENT_COMPARISON.md"
    ...
    documents, rotations, skipped = load_documents(args.data_root, manifest)
    if not documents:
        raise RuntimeError("No eligible development documents found")
    baseline = evaluate_variant(documents, "phase11_5")
    candidate = evaluate_variant(documents, "ocr_ho_v2_007")
    report = {
        "schemaVersion": "ocr-ho-v2-007-evaluation/1.0.0",
        ...
        "metrics": {"baseline_phase11_5": baseline, "ocr_ho_v2_007": candidate},
