...
class UserOCRService:
    ...
    def apply_phase12(
        ...
        classification = classify_hr_document(
            canonical_document,
            result.get("document", {}).get("documentType"),
        )
        extraction = extract_hr_document(
            canonical_document,
            classification,
        )
        business_json = build_business_json(
            result["sessionId"],
            canonical_document,
            classification,
            extraction,
        ...
        phase12_dir = session_dir / "phase12"
        phase12_dir.mkdir(parents=True, exist_ok=True)
        canonical_path = phase12_dir / "canonical_document.json"
        business_path = phase12_dir / "business.json"
        idp_path = phase12_dir / "idp_result.json"
        ...
        idp_result = {
            ...
            "businessJson": "phase12/business.json",
        }
        idp_path.write_text(
        ...
        result["phase12"] = {
            ...
            "durationMs": duration_ms,
            "downloads": {
                "canonicalDocument": "phase12/canonical_document.json",
                "idpResult": "phase12/idp_result.json",
                "businessJson": "phase12/business.json",
            },
        }
        result["processing"]["phase12DurationMs"] = duration_ms
        return result
